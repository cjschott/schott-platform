...
# Generation 18: the duplicate-result gate, deployed.
#
# WHAT THIS DEPLOYS. TWO objects, in ONE coherence group:
#
#   T  the terminal-result authority.
#      `evidence.py` gains the one reader that answers "does this invocation
#      already have a governed terminal result", and `coordinator.py` asks it
#      BEFORE `supervisor.execute` rather than after.
#
# WHAT WENT WRONG. `ExecutionSupervisor.execute` launches the privileged helper
# on its first line. The guard refusing a second terminal result ran after that
# call returned, so a re-execute of an invocation that already has a `CRES`
# would run the transition, create the container, transfer handoff ownership,
# run the provider and dispose of it -- and only then decline to write the
# result down. An execution that happened and was never recorded. G11-BC-I found
# it on CINV-000002, whose CRES-000001 is final.
#
# Two fail-open holes were closed on the way. The guard matched only
# `attempt_number == 1`, so a result carrying any other value blocked nothing;
# and `list_records` silently skips a file that is not a mapping, so a corrupted
# `CRES` would vanish and the reader would answer "no result" for an invocation
# that has one.
#
# WHY THESE TWO ARE ONE GROUP. `coordinator.py` imports
# `require_no_terminal_result` from `evidence.py` at module load, and
# `evidence.py` is the only definition of it. Neither object is a valid complete
# Generation 18 on its own: `coordinator.py` alone cannot import, and
# `evidence.py` alone defines a gate nobody calls -- a host that looks corrected
# and is not. They are therefore ONE group, and `require_group_coherence`
# refuses any committed state holding one at each generation.
#
# PUBLICATION ORDER IS THE SAFETY PROPERTY, AND IT IS DERIVED, NOT INHERITED.
# Generation 17's order came from `helpers.py` closing execution first; that
# reasoning does not transfer, because neither object here is a readiness
# authority and NEITHER closes execution. So both orders were measured, by
# importing `tools.capability.cli` against all four mixes:
#
#   evidence  coordinator  observed
#   --------  -----------  ----------------------------------------------------
#   old       old          gate not called; record guard fail-open   (Gen 17)
#   NEW       old          gate not called; record guard HARDENED    <- order A
#   old       NEW          ImportError: cannot import name
#                          'require_no_terminal_result'              <- order B
#   NEW       NEW          gate called; record guard hardened        (Gen 18)
#
# Order A's only intermediate is STRICTLY SAFER than the accepted Generation-17
# baseline: the same execution behaviour, with a hardened recording guard. Order
# B's intermediate breaks every `tools.capability.cli` command -- including
# `recover`, the one an operator needs to resolve an interrupted transaction.
# That is fail-closed for execution and fail-closed for the recovery surface
# too, which is worse, not better.
#
# So: `evidence.py` FIRST. `require_fail_closed_first` holds it as a checked
# property, and `rollback` restores in reverse -- `coordinator.py` back first --
# so the old/NEW ImportError state is unreachable from either direction.
#
# EXECUTION STAYS OPEN THROUGH THIS GENERATION, DELIBERATELY. Unlike Generation
# 17, no readiness authority moves: `helpers.py` is untouched, compatibility
# stays `compatible`, blocking stays 0 and supervision_ready stays true at every
# state. There is nothing to reopen and no helper ceremony to follow. A host
# reporting `incompatible` after this ran would mean something outside this
# matrix moved.
#
...
# not one byte of Fabric, Trust, or invocation history. CRES-000001 is not
# reinterpreted and nothing is migrated or backfilled.
#
# Usage:
#   install-generation-18.sh --verify-source      reviewed bytes only; reads no installed state
#   install-generation-18.sh --verify             is this host a Generation-17 host ready for it?
#   install-generation-18.sh --install            the transaction
#   install-generation-18.sh --verify-installed   is this host a whole Generation 18?
#   install-generation-18.sh --recover            resolve an interrupted transaction
#
# Test-only:
...
# The accepted Generation-17 source authority, and the baseline this transaction
# requires the host to be at.
GEN17_COMMIT="15a8c738f97394a4f114070c011db22562466ed6"
...
# This transaction's own namespace. Generation 17's retained journal at
# /root/kyri-gen17-transaction is predecessor evidence: it records how the host
...
BASELINE_LIBRARY_EVIDENCE="/root/kyri-gen17-library-digests.txt"
BASELINE_HELPER_EVIDENCE="/root/kyri-gen17-helper-digests.txt"
GEN18_LIBRARY_EVIDENCE="/root/kyri-gen18-library-digests.txt"
GEN18_HELPER_EVIDENCE="/root/kyri-gen18-helper-digests.txt"
...
# create them; never written. Generation 17 does not need them to install.
...
if [[ -n "${FIXTURE}" ]]; then
  ...
  GEN18_LIBRARY_EVIDENCE="${FIXTURE}${GEN18_LIBRARY_EVIDENCE}"
  GEN18_HELPER_EVIDENCE="${FIXTURE}${GEN18_HELPER_EVIDENCE}"
...
# --- the two generation-18 objects, pinned both ways ------------------------
#
# source | target | mode | operation | gen17-sha256 | gen18-sha256 | group
#
# Both targets sit in a directory that already exists at Generation 17, so there
# is no directory to create and none to remove on rollback.
#
# THE ONE GROUP, AND WHY IT IS ONE.
#
#   T  the terminal-result authority. `evidence.py` owns the reader; the gate in
#      `coordinator.py` is its only caller, and the import is resolved at module
#      load. A committed state holding one member at Generation 17 and the other
#      at 18 is either an ImportError on every CLI command or a gate that is
#      defined and never consulted. Neither is a host anybody should be left
#      with, which is exactly what a coherence group is for.
#
# The letter follows the convention the existing installers use -- a mnemonic
# for the capability, not a sequence position: V is the runtime-side
# verification surface, R supervised recovery discovery, H helper declaration
# and refusal reporting, A the container-runtime binding. T is the
# terminal-result authority, and it is unused across Generations 13 to 17.
#
# There is no CARRYOVER: every member of T is a row here. The CARRYOVER list
# below is therefore empty, and it is kept rather than removed so that adding
# one later is a visible edit.
#
# source | target | mode | operation | gen17-sha256 | gen18-sha256 | group
MATRIX=(
# --- T: the terminal-result authority. evidence.py FIRST -- it defines the
#        reader coordinator.py imports at module load, so the reverse order is
#        an ImportError on every CLI command. Measured, see the header.
"tools/capability/evidence.py|${LIBRARY_ROOT}/tools/capability/evidence.py|0444|REPLACE|25f65bd345efc84faadfefff635d50b850df9362ae20130f47b55fc9cf8588b8|a571ad02ace56dbb93a5cc9385a4b2cca4e5c1922fa16bfa9f59848a25a386f6|T"
"tools/capability/coordinator.py|${LIBRARY_ROOT}/tools/capability/coordinator.py|0444|REPLACE|b72e7e2576095c96ffd5b4a3a48acc2fed7c1852b631a5f6f7d691e0bf8603c0|acb80cb93084b2b196d6b806b278128458be8947a9575eac7c7c509e9f045585|T"
)
...
# Every group Generation 17 named is still spelled here, even though this
...
#
# Generation 17 put a row in group A and never added a case for it, so a split
...
# The modules that must NOT be installed, carried forward from Generation 18
...
injected_at() {
  [[ -n "${FIXTURE}" && "${KYRI_GEN18_FAIL_AT:-}" == "$1" ]]
}
...
unwind_preparation() {
  ...
  printf 'unwound  preparation: %d staged object(s) removed; the host is at Generation 17\n' \
    "${removed}" >&2
}
...
require_group_coherence() {
  ...
  for group in $(matrix_groups); do
    ...
    if (( at_baseline > 0 && at_target > 0 )); then
      bad "group ${group} ($(group_name "${group}")) is split: ${at_baseline} object(s) at Generation 18 and ${at_target} at Generation 18"
      split=$((split + 1))
    fi
  done
  (( split == 0 )) \
...
require_repository() {
  ...
  git_as_owner cat-file -e "${COMMIT}^{commit}" 2>/dev/null \
    || halt "the reviewed Generation-18 commit ${COMMIT} is not in this repository"
  git_as_owner merge-base --is-ancestor "${COMMIT}" HEAD 2>/dev/null \
    || halt "the reviewed Generation-18 commit ${COMMIT} is not an ancestor of HEAD (${head_now})"
  git_as_owner merge-base --is-ancestor "${GEN17_COMMIT}" "${COMMIT}" 2>/dev/null \
    || halt "the Generation-17 authority is not an ancestor of the Generation-18 authority"
...
require_source_digests() {
  ...
  (( drift == 0 )) || halt "the reviewed commit does not carry the pinned Generation-18 surface"
  local checked_n
  checked_n="$(matrix_count)"
  ok "${checked_n} Generation-18 source $(plural "${checked_n}" object objects) match the reviewed commit ${COMMIT}"
}
...
require_closed_closure() {
  ...
  # What the predecessor provides, decided from the reviewed Generation-17
  ...
  while IFS= read -r candidate; do
    ...
    if git_as_owner cat-file -e "${GEN17_COMMIT}:${source}" 2>/dev/null; then
      carried+="${candidate}"$'\n'
    fi
  done < <(printf '%s\n' "${computed}")
...
# Generation 17 read only the G11-AX ceremony here, because that was the only
...
#     -- the last ceremony accepted BEFORE Generation 18 that governs it,
#        or Generation-17 evidence when none does
#   PLUS the target of every ceremony accepted AFTER Generation 18 that governs it
#
# BOTH HELPER CEREMONIES ARE NOW "BEFORE", AND THAT IS THE WHOLE DIFFERENCE FROM
# GENERATION 15'S COPY OF THIS LIST. G11-BB was still pending when Generation 17
...
# so for Generation 18 there is exactly ONE accepted state per object again:
...
# accepted ceremony is scheduled to run after Generation 18. If one is later
...
# Generation 17 rather than an omission. G11-BC-E ran and was accepted, so it
...
require_baseline() {
  ...
  [[ "${count}" -eq "${expected_baseline}" ]] \
    || halt "the installed library holds ${count} objects, expected the Generation-17 ${EXPECTED_LIBRARY_FILES_BASELINE} plus ${helpers_present} published helper module(s)"

  [[ -f "${BASELINE_LIBRARY_EVIDENCE}" ]] \
    || halt "the Generation-17 library evidence at ${BASELINE_LIBRARY_EVIDENCE} is missing"
  [[ -f "${BASELINE_HELPER_EVIDENCE}" ]] \
    || halt "the Generation-17 helper evidence at ${BASELINE_HELPER_EVIDENCE} is missing"
  ...
  while IFS= read -r file; do
    ...
    # is still judged against Generation-17 evidence and still refuses.
    if ! accepted="$(accepted_library_digest "${relative}")"; then
      bad "installed object ${relative} is absent from the Generation-17 evidence"
      drift=$((drift + 1)); continue
    fi
  ...
  while IFS= read -r recorded_relative; do
    ...
    [[ -f "${LIBRARY_ROOT}/${recorded_relative}" ]] \
      || { bad "the Generation-17 evidence records ${recorded_relative}, which is not installed"; drift=$((drift + 1)); }
  done < <(sed -n 's#^[0-9a-f]\{64\}  /usr/lib/kyri/python/##p' "${BASELINE_LIBRARY_EVIDENCE}")

  (( drift == 0 )) || halt "the installed runtime is not the accepted Generation-17 baseline"
  ok "the installed runtime is exactly the accepted Generation-17 baseline (${count} objects)"
}
...
require_target_state() {
  ...
  if (( UNKNOWN_COUNT > 0 )); then
    ...
    for target in "${UNKNOWN_TARGETS[@]}"; do
      bad "UNKNOWN object at ${target}: neither the Generation-17 baseline nor the Generation-18 target"
    done
...
report_transaction_residue() {
  ...
  if (( extra == 0 )); then
    ...
    note "${extra} transaction artefact(s) remain from a cleanup that did not finish; Generation 18 is installed and unaffected. Remove them with --recover or by hand."
  fi
}
...
# which is Generation-17 behaviour with a HARDENED recording guard, strictly
...
FAIL_CLOSED_FIRST="tools/capability/evidence.py"

# THE ORDER, HELD AS A CHECKED PROPERTY.
#
# Generation 17 named this after the object that closes execution. Nothing here
...
require_fail_closed_first() {
  ...
  second="$(field "${MATRIX[1]}" 0)"
  [[ "${first}" == "${FAIL_CLOSED_FIRST}" ]] \
    || halt "the first published object is ${first}, not ${FAIL_CLOSED_FIRST}: the importing module would be published against a definition that does not exist yet"
  [[ "${second}" == "tools/capability/coordinator.py" ]] \
    || halt "the second published object is ${second}, not the importing module"
  [[ "$(field "${MATRIX[0]}" 6)" == "T" && "$(field "${MATRIX[1]}" 6)" == "T" ]] \
    || halt "both objects must be in coherence group T: neither is a valid complete Generation 18 alone"
  ok "the defining module publishes first and the importing module second, both in group T"
}

# The coherence-group member this generation does NOT move, checked on both
# sides. Empty for Generation 18 -- every member of both groups is a row -- but
...
report_execution_readiness() {
  ...
  if [[ ! -f "${COORDINATOR_IDENTITY}" ]]; then
    note "the coordinator identity authority is not installed; Generation 18 installs without it"
    ready=0
  fi
  if [[ ! -f "${EXECUTION_IDENTITY}" ]]; then
    note "the execution identity authority is not installed; Generation 18 installs without it"
    ready=0
  fi
  if (( ready == 1 )); then
    note "both deployment identity authorities are installed"
  else
    note "this host will be at Generation 18 and NOT execution-ready: the deployment identity ceremony is a separate one, and the runtime refuses execution until it has run"
  fi
...
prepare() {
  ...
  for row in "${MATRIX[@]}"; do
    ...
    sync_path "${prepared}"
    injected_at staged && halt "injected failure after staging a Generation-18 object"
  done
...
commit_targets() {
  ...
  for row in "${MATRIX[@]}"; do
    ...
    if [[ -n "${FIXTURE}" && "${KYRI_GEN18_FAIL_AT:-}" == "${index}" ]]; then
  ...
  injected_at postcommit \
    && bad "injected failure immediately after COMMITTED; Generation 18 stands"
  OUTCOME="COMMITTED"
...
rollback() {
  ...
  # Generation 17 needed -- a fail-closed authority restored LAST, so the
  # predecessor declaration never returns while a successor object is still
  # published -- and a multi-row Generation 18 would need it back. Rewriting it
  ...
  if (( BASELINE_COUNT == ${#MATRIX[@]} )); then
    ...
    ok "ROLLBACK complete: ${rolled_n} $(plural "${rolled_n}" target targets) back at Generation 17 (${removed} restored or removed)"
  else
    journal_write ROLLING_BACK
...
recover() {
  ...
  if (( UNKNOWN_COUNT > 0 )); then
    local target
    for target in "${UNKNOWN_TARGETS[@]}"; do
      bad "UNKNOWN bytes at ${target} (neither the Generation-17 baseline nor the Generation-18 target, and not absent)"
    done
  ...
  if (( TARGET_COUNT == ${#MATRIX[@]} )); then
    journal_write COMMITTED
    OUTCOME="COMMITTED"
    ok "recovery: the complete Generation-18 set is already installed"
    return 0
  fi
  if (( BASELINE_COUNT == ${#MATRIX[@]} )); then
    journal_write ROLLED_BACK
    OUTCOME="ROLLED_BACK"
    ok "recovery: no Generation-18 object was published; the host is at Generation 17"
    return 0
  fi
...
write_evidence() {
  [[ -f "${BASELINE_LIBRARY_EVIDENCE}" && -f "${BASELINE_HELPER_EVIDENCE}" ]] \
    || halt "Generation-17 evidence vanished during installation"
  if injected_at evidence; then
    bad "injected failure while writing Generation-18 evidence; Generation 18 stands"
    return 0
  fi
  find "${LIBRARY_ROOT}" -type f -name '*.py' -print0 \
    | sort -z | xargs -0 sha256sum > "${GEN18_LIBRARY_EVIDENCE}.writing"
  local row target
  for row in "${MATRIX[@]}"; do
    target="$(field "${row}" 1)"
    grep -q "${target}\$" "${GEN18_LIBRARY_EVIDENCE}.writing" \
      || { rm -f "${GEN18_LIBRARY_EVIDENCE}.writing"
           halt "the Generation-18 evidence does not record ${target}"; }
  done
  {
    printf 'commit %s\n' "${COMMIT}"
    printf 'baseline_commit %s\n' "${GEN17_COMMIT}"
    printf 'predecessor generation 17\n'
    printf 'transaction %s\n' "${TRANSACTION_ID}"
  ...
  } > "${GEN18_HELPER_EVIDENCE}.writing"
  chmod 0400 "${GEN18_LIBRARY_EVIDENCE}.writing" "${GEN18_HELPER_EVIDENCE}.writing"
  sync_path "${GEN18_LIBRARY_EVIDENCE}.writing"
  sync_path "${GEN18_HELPER_EVIDENCE}.writing"
  mv -f "${GEN18_LIBRARY_EVIDENCE}.writing" "${GEN18_LIBRARY_EVIDENCE}"
  mv -f "${GEN18_HELPER_EVIDENCE}.writing" "${GEN18_HELPER_EVIDENCE}"
  sync_path "${GEN18_LIBRARY_EVIDENCE}"
  sync_path "${GEN18_HELPER_EVIDENCE}"
  ok "Generation-18 evidence written; Generation-17 evidence preserved"
}
...
cleanup_transaction_artifacts() {
  ...
  if injected_at cleanup; then
    bad "injected cleanup failure after COMMITTED; Generation 18 remains installed"
    return 0
  fi
  for row in "${MATRIX[@]}"; do
    target="$(field "${row}" 1)"
...
verify_installed_set() {
  ...
  [[ "${count}" -eq "${expected_target}" ]] \
    || bad "the installed library holds ${count} objects, expected the Generation-18 ${EXPECTED_LIBRARY_FILES_TARGET} plus ${helpers_present} published helper module(s)"
  (( FAILURES == 0 )) \
    && ok "all $(matrix_count) Generation-18 changed objects correspond to the reviewed commit ${COMMIT}"
}
...
#
# The accepted authority is Generation 17 PLUS what accepted ceremonies
...
# what refused a COMMITTED Generation-18 transaction: the four G11-AX
...
verify_unchanged_surface() {
  ...
  while IFS= read -r file; do
    ...
    relative="${file#"${LIBRARY_ROOT}"/}"
    if ! accepted="$(accepted_library_digest "${relative}")"; then
      bad "installed object ${relative} is not accounted for by the Generation-17 evidence or any accepted ceremony, and is not a declared Generation-18 target"
      drift=$((drift + 1)); continue
    fi
    read -r authority recorded <<<"${accepted}"
    observed="$(digest_of "${file}")"
    ...
    if [[ "${authority}" == "ceremony" ]]; then
      ...
      bad "${relative} changed: ${observed} but Generation-17 evidence records ${recorded}"
    fi
    drift=$((drift + 1))
  done < <(find "${LIBRARY_ROOT}" -type f -name '*.py' | sort)
  ...
  while IFS= read -r recorded_relative; do
    [[ -n "${recorded_relative}" ]] || continue
    [[ -f "${LIBRARY_ROOT}/${recorded_relative}" ]] \
      || { bad "the Generation-17 evidence records ${recorded_relative}, which is no longer installed"; drift=$((drift + 1)); }
  done < <(sed -n 's#^[0-9a-f]\{64\}  /usr/lib/kyri/python/##p' "${BASELINE_LIBRARY_EVIDENCE}")

  (( drift == 0 )) \
    && ok "every carried-over runtime object is exactly its accepted predecessor -- Generation 17, plus the $(helper_ceremony_library_rows "${CEREMONIES_BEFORE_THIS_GENERATION[@]}" "${CEREMONIES_AFTER_THIS_GENERATION[@]}" | wc -l) object(s) an accepted ceremony published after it -- and nothing was removed"
}
...
--verify-source)
  ...
  printf '\n'
  printf 'Generation 18 source verification: all checks passed. %s object(s) would change (%s REPLACE, %s CREATE).\n' \
...
--verify)
  ...
  classify_all
  if (( TARGET_COUNT == ${#MATRIX[@]} )); then
    note "all $(matrix_count) targets are already at Generation 18; use --verify-installed to audit the installed generation"
    require_gates_closed
    verify_excluded_absent
    require_group_coherence
    note "authority namespace fingerprint: $(authority_fingerprint)"
    printf '\n'
    printf 'Generation 18 / supervised execution runtime verify: already installed.\n'
    exit 0
  fi
  ...
  if (( BASELINE_COUNT == ${#MATRIX[@]} )); then
    ok "the host is at Generation 17 and ready for the Generation-18 installation: $(matrix_count_of REPLACE) REPLACE, $(matrix_count_of CREATE) CREATE, $(matrix_count) changed objects across $(matrix_groups | wc -l) coherence groups, object count ${EXPECTED_LIBRARY_FILES_BASELINE} -> ${EXPECTED_LIBRARY_FILES_TARGET}"
  else
...
--install)
  ...
  if [[ "${state}" == "NONE" ]]; then
    ...
    classify_all
    if (( TARGET_COUNT == ${#MATRIX[@]} )); then
      ok "Generation 18 is already installed: nothing to do"
      exit 0
    fi
    prepare
    verify_prepared_set
  ...
  if [[ "${OUTCOME}" == "COMMITTED" ]]; then
    ...
    cleanup_transaction_artifacts
    require_group_coherence
    bad "the transaction rolled back: the host is at Generation 17 and nothing was installed"
  else
...
--verify-installed)
  ...
  [[ -f "${GEN18_LIBRARY_EVIDENCE}" ]] \
    || bad "the Generation-18 library evidence is missing"
  [[ -f "${GEN18_HELPER_EVIDENCE}" ]] \
    || bad "the Generation-18 helper evidence is missing"
  [[ -f "${BASELINE_LIBRARY_EVIDENCE}" ]] \
    || bad "the Generation-17 evidence was not preserved"
  [[ -f "${BASELINE_HELPER_EVIDENCE}" ]] \
    || bad "the Generation-17 helper evidence was not preserved"
  report_transaction_residue
  note "authority namespace fingerprint: $(authority_fingerprint)"
  ;;
...
--recover)
  ...
  if [[ "${OUTCOME}" == "COMMITTED" ]]; then
    ...
    cleanup_transaction_artifacts
    require_group_coherence
    bad "recovery rolled the transaction back: the host is at Generation 17 and Generation 18 is not installed"
  else
...
printf '\n'
if (( FAILURES == 0 )); then
  printf 'Generation 18 / supervised execution runtime %s: all checks passed.\n' "${MODE#--}"
else
  printf 'Generation 18 / supervised execution runtime %s FAILED: %d\n' "${MODE#--}" "${FAILURES}" >&2
  exit 1
fi
