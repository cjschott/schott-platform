...
# The live count MINUS every library-root pathname a later generation creates,
# found by name rather than listed. The raw live count was the comparison until
# Generation 19 published abandonment.py, at which point the reconstruction was
# correct and the assertion about it was not: a fixture rebuilt from the
# Generation-15 tree cannot hold an object that did not exist then.
successor_created_since_15() {
  local installer number row target created=0
  for installer in "${ROOT}"/provisioning/execution/install-generation-*.sh; do
    ...
    while IFS= read -r row; do
      ...
      # Declared by a later generation is not the same as PRESENT. Generation 20
      # is declared and not installed, so subtracting its create would describe a
      # host that does not exist. Only what the live tree actually holds counts.
...
expected_count=$(( live_count - $(successor_created_since_15) ))
